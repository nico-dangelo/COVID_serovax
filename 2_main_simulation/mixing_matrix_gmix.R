...
#               nrow = 3, 
#               dimnames = list(x=c("0-17","18-59","50+"), y=c("0-17","18-59","50+")))
MUestimates_all_locations_US <- readxl::read_excel("~/COVID_serovax/2_main_simulation/MUestimates_all_locations_2.xlsx", sheet = "United States of America", col_names = FALSE)

rownames(MUestimates_all_locations_US) <- colnames(MUestimates_all_locations_US) <- c("0-4","5-9","10-14","15-19","20-24","25-29","30-34","35-39","40-44","45-49","50-54","55-59","60-64","65-69","70-74","75+")
...
CM_u <-
  matrix(
    c(
      sum(MUestimates_all_locations_US[1:4, 1:4]),
      sum(MUestimates_all_locations_US[5:13, 1:4]),
      sum(MUestimates_all_locations_US[14:16, 1:4]),
      sum(MUestimates_all_locations_US[1:4, 5:13]),
      sum(MUestimates_all_locations_US[5:13, 5:13]),
      sum(MUestimates_all_locations_US[14:16, 5:13]),
      sum(MUestimates_all_locations_US[1:4, 14:16]),
      sum(MUestimates_all_locations_US[5:13, 14:16]),
      sum(MUestimates_all_locations_US[14:16, 14:16])
    ),
    nrow = 3,
    ncol = 3,
    dimnames = list(
      x = c("0-19", "20-64", "65-80"),
      y = c("0-19", "20-64", "65-80")
    ),
    byrow = F
  )
